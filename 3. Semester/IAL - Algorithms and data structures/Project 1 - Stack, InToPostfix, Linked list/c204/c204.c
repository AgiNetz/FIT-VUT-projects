...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen )
{
    while(!stackEmpty(s))
    {
        char c;
        stackTop(s, &c);
        /*Pokud jsme narazili na levou zavorku, tak pop
        jinak pridat vrchol zasobniku do vystupniho retezce + pop*/
        if(c != '(')
        {
            postExpr[*postLen] = c;
            (*postLen)++;
...
void doOperation ( tStack* s, char c, char* postExpr, unsigned* postLen )
{
    if(!stackEmpty(s))
    {
        char t;
        stackTop(s,&t);
        //Je na vrcholu zasobniku '(' nebo operator s nizsi prioritou?
        if((t == '(') || ((c == '*' || c == '/') && (t == '+' || t == '-')))
            stackPush(s,c);
        else
        {
            //Odstranujeme a davame na vystup operatory se stejnou/vyssi prioritou
            stackPop(s);
            postExpr[*postLen] = t;
            (*postLen)++;
            //Opakujeme proces
...
char* infix2postfix (const char* infExpr)
{
	...
	while(infExpr[inIndex] != 0)
	{
        char c = infExpr[inIndex];
        if((c >= '0' && c <= '9') || (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) //je c alfanumericky znak?
        {
            output[outIndex] = c;
            outIndex++;
        }
            ...
            untilLeftPar(&s,output,&outIndex); //Zpracovani zatvorky
        else if (c == '=')
        {
            //Dokonceni vyrazu a vyprazdneni zasobniku
            while(!stackEmpty(&s))
            {
                stackTop(&s,&c);
                output[outIndex] = c;
            ...
            doOperation(&s,c,output,&outIndex);
        inIndex++;
	}
	return output;
}
